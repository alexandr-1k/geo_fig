...
std::optional<Shape> MakeCircle(const std::vector<double> &v) {
    return RequireSize(v, 3)
        .and_then([](const std::vector<double> &vals) { return RequirePositive(vals[2]); })
...
std::optional<Shape> MakeLine(const std::vector<double> &v) {
    return RequireSize(v, 4).transform([&v](const std::vector<double> &) { return Line{{v[0], v[1]}, {v[2], v[3]}}; });
}
...
std::optional<Shape> MakeTriangle(const std::vector<double> &v) {
    return RequireSize(v, 6).transform(
        [&v](const std::vector<double> &) { return Triangle{{v[0], v[1]}, {v[2], v[3]}, {v[4], v[5]}}; });
}
...
std::optional<Shape> MakeRectangle(const std::vector<double> &v) {
    return RequireSize(v, 4)
        .and_then([&v](const auto &) { return RequirePositive(v[2]); })  // width
        .and_then([&v](double) { return RequirePositive(v[3]); })        // height
        .transform([&v](double) { return Rectangle{{v[0], v[1]}, v[2], v[3]}; });
}
/**
...
std::optional<Shape> MakePolygon(const std::vector<double> &v) {
    ...
    // auto sides_opt = ParseDouble(std::to_string(v[3]));  // v[3] — double, но sides — целое
    // if (!sides_opt.has_value())
    //     return std::nullopt;
    // int sides = static_cast<int>(v[3]);
    // if (sides != v[3] || sides < 3)
    //     return std::nullopt;  // должно быть целым и >=3
    // return RegularPolygon{{v[0], v[1]}, v[2], sides};

    return RequireSize(v, 4)
        .and_then([&v](const auto &) { return RequirePositive(v[2]); })        // radius
        .and_then([&v](double) { return ParseDouble(std::to_string(v[3])); })  // check if sides is double
        .and_then([&v](double) { return RequireIntegerAtLeast(v[3], 3); })     // sides
...
std::optional<size_t> FindHighestShape(std::span<const Shape> shapes) {
    ...
    size_t index = std::distance(shapes.begin(), max_it);
    return index;
}
}  // namespace geometry::utils